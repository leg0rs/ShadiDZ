...
export default function LoginPage() {
	...
	const handleGoogleSignIn = async () => {
		...
		// Тут лоадинг не снимаем, так как уйдем на редирект гугла
	};
	...
	const handleEmailSignIn = async () => {
		...
		if (error) {
			...
			// Успех! Better auth сам поставит куки. Делаем редирект.
			router.push('/');
			router.refresh(); // Обновляем роутер, чтобы middleware увидел куки
		}
	};
			...
			<div className="w-full max-w-md bg-card rounded-xl shadow-lg p-8 border border-border">
				...
				<div className="flex flex-col gap-4">
					...
					<input
						...
						disabled={isLoading}
						className="w-full border border-input p-3 rounded-lg focus:ring-2 focus:ring-ring focus:border-transparent outline-none transition disabled:bg-muted bg-background text-foreground"
					/>
